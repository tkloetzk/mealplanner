...
describe('MealStore Integration Tests', () => {
  ...
  describe('Food Selection', () => {
    it('handles food selection for different categories', async () => {
      ...
      const selections = result.current.selections[mockKids[0].id]['monday']['breakfast'];
    ...
    it('toggles food selection when same food is selected again', async () => {
      ...
      let selections = result.current.selections[mockKids[0].id]['monday']['breakfast'];
      expect(selections?.proteins?.id).toBe(testFood.id);
      ...
      selections = result.current.selections[mockKids[0].id]['monday']['breakfast'];
    ...
    it('handles condiments as array', async () => {
      ...
      const selections = result.current.selections[mockKids[0].id]['monday']['lunch'];
      ...
      const updatedSelections = result.current.selections[mockKids[0].id]['monday']['lunch'];
  ...
  describe('Serving Adjustments', () => {
    it('adjusts serving sizes for regular foods', async () => {
      ...
      // Adjust servings
      await act(async () => {
        await result.current.handleServingAdjustment('proteins', testFood.id, 2);
      });
    ...
    it('adjusts serving sizes for condiments', async () => {
      ...
      // Adjust servings
      await act(async () => {
        await result.current.handleServingAdjustment('condiments', condiment.id, 3);
      });
  ...
  describe('Selectors', () => {
    it('useCurrentMealSelection returns current meal selections', () => {
      ...
      const { result: selectorResult } = renderHook(() => useCurrentMealSelection());
    ...
    it('useMealNutrition calculates nutrition correctly', async () => {
      const { result: storeResult } = renderHook(() => useMealStore());
      const { result: nutritionResult } = renderHook(() => useMealNutrition('breakfast'));
